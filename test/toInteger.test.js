...
test("converts a valid string representations of numbers to integers", () => {
    expect(toInteger("10")).toBe(10);
    expect(toInteger("3.14")).toBe(3);
    expect(toInteger("-1.23")).toBe(-1);
});

test("handles invalid string representations of numbers without unexpected results", () => {
    expect(() => {
        toInteger("123abc")
    }).not.toThrow();
});
...
test("handles arrays without unexpected results", () => {
    expect(() => {
        toInteger([0, 1, 2, 3])
    }).not.toThrow();
});

test("handles objects without unexpected results", () => {
    const obj = { x: 1 };
    expect(() => {
        toInteger(obj)
    }).not.toThrow();
